...
后续更新：用户已提供真实 Vulkan 捕获，现已通过官方 RenderDoc 离线回放并导出实际
Shader、常量和资源绑定。部分全局参数布局已有对应证据；见
[首次捕获分析](tifuluosi-capture-intake-20260917.md)。下文的「尚无 RDC」是此前阶段记录，
不再代表当前数据状态；Unity 视觉还原仍未完成。
