...
## 2026-09-26 后续核查：用户提供的 `Downloads/models`

- `C:/Users/Administrator/Downloads/models` 有两套提弗洛斯 PMX 和一套原始 FBX。FBX 与 `Assets/Typhoeus/chr_0034_typhoea_uimodel.fbx` SHA-256 相同，不是新的缺失素材。
- `Typhoeus.pmx`（SheepyLord，354 骨/59,753 顶点）和 `提弗洛斯.pmx`（茶叶味香皂，1,182 骨/65,427 顶点）均能按标准化骨骼名匹配当前 UNFORGIVEN VMD 的 **52/52 条轨道**；两者各有 4 个 IK 控制骨。复核命令见 `Tools/inspect_mmd_compat.py --help`。这只证明命名覆盖，不证明姿态、IK、裙摆物理和镜头一致。
- 当前 VMD 头部写的是 `1.トレモデル1.04(バ…`，两套 PMX 都不是其原始动作源模型。禁止把“目标 PMX 轨道全覆盖”写成“已找到源 PMX”。两套 PMX 骨架比例也不同：左手首 X 分别约 4.19/4.84 MMD 单位，而当前播放器固定 `StandardMmd()` 的左手首 X 是 5.6；需要以真实模型骨架做 A/B 评估，不能仅调放大系数。
- 茶叶味香皂模型的 ReadMe 禁止二次配布和拆件改造；这轮未复制/提交 PMX、贴图、骨架导出数据，仅只读检查其结构。保留官方解包网格作最终渲染目标。
- 补齐 `MmdRetargetProfile.RoleBindings` 的 30 个指骨角色（24–53）。新烟测同时检查全部角色存在及手指姿态变化，`maxFingerAngle=113.998°`。此前角色映射确实缺失，已修复。
- 新增 `BakeMesh` 的根局部顶点位移门禁，五时点 P95 最大 **0.524349 m**，Unity 烟测通过；证明网格确实随骨架显著变形，否定“只有 Transform 在转、网格完全不动”的简单解释。四条上臂/大腿方向与当前源求解器的方向余弦约 1.0；这是内部一致性，不能替代对原始 PMX/MMD 的外部真值验证。
- Python 128/128 测试通过；Unity 2022.3.30f1 批处理五时点烟测退出码 0。当前仍未达到“完美 MMD”：`StandardMmd()` 是手工通用源骨架，缺少动作原始模型的静止姿态/附加骨/物理真值；脚/布料接地与全片逐帧品质均未过关。

下一阶段的正确实验：在隔离支线做 **PMX 源骨架 / 标准源骨架 A/B**，同一 VMD 同时驱动各自源模型并对齐手、脚、髋轨迹和关键帧截图；优先查找这条动作真正的 `トレモデル1.04` 源 PMX。如找不到，以用户现有 PMX 作为近似参照，明确标注非精确源。只有证明一个方案在多个动作的关节轨迹和接地指标上更好，才替换默认运行时；不要直接把第三方 PMX 网格并入官方渲染场景。
