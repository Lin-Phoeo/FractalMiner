...
def shadow_block(blocks):
    matches = [block for block in blocks if block['size'] == SHADOW_CBUFFER_BYTES]
    if not matches:
        raise ValueError('No constant block of {} bytes at the resolve event; found {}'.format(
            SHADOW_CBUFFER_BYTES, [(b['name'], b['size']) for b in blocks]))
    if len(matches) != 1:
        raise ValueError('Ambiguous shadow cbuffer: {} blocks of {} bytes'.format(
            len(matches), SHADOW_CBUFFER_BYTES))
    return matches[0]
...
def select_constants(block):
    """Read the character shadow fields by byte offset, never by stripped name."""
    at_offset = {}
    for offset, variable in child_offsets(block['variables']):
        at_offset.setdefault(offset, variable)
    result = {}
    for plan in CONSTANT_PLAN:
        variable = at_offset.get(plan['offset'])
        if variable is None:
            raise ValueError('{}: no reflection variable starts at byte {} (c{}); the shadow cbuffer'
                             ' layout drifted, refusing to guess'.format(
                                 plan['name'], plan['offset'], plan['register']))
        if plan['count'] == 1:
            if variable.members:
                raise ValueError('{}: expected one float{}x{} at c{}, got an array of {}'.format(
                    plan['name'], plan['rows'], plan['columns'], plan['register'], len(variable.members)))
            result[plan['name']] = [leaf_values(variable, plan)]
            continue
        if len(variable.members) != plan['count']:
            raise ValueError('{}: expected {} entries at c{}, got {}'.format(
                plan['name'], plan['count'], plan['register'], len(variable.members)))
...
def collect(rd, controller, output, constants_only):
    ...
    block = shadow_block(pipeline_constant_blocks(rd, controller, state))
    ...
    write_new_json(output / 'constants.json', dict(
        event=RESOLVE_EVENT, cbuffer=dict(name=block['name'], resource=block['resource'],
        offset=block['offset'], size=block['size']), constants=constants))
    return dict(event=RESOLVE_EVENT, directional_event=DIRECTIONAL_EVENT,
                output_target=target, cbuffer_size=block['size'],
                bindings=bindings, constants=constants, textures=records)
